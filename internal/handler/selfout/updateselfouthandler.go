...
func UpdateSelfoutHandler(svcCtx *svc.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req types.UpdateSelfoutReq
		if err := bind.Parse(c, &req); err != nil {
			httpresp.Fail(c, err)
			return
		}

		l := selfout.NewUpdateSelfoutLogic(c.Request.Context(), svcCtx)
		resp, err := l.UpdateSelfout(&req)
		if err != nil {
			httpresp.Fail(c, err)
		} else {
