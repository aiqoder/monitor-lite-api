...
func DelSelfoutHandler(svcCtx *svc.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req types.DelSelfoutReq
		if err := bind.Parse(c, &req); err != nil {
			httpresp.Fail(c, err)
			return
		}

		l := selfout.NewDelSelfoutLogic(c.Request.Context(), svcCtx)
		resp, err := l.DelSelfout(&req)
		if err != nil {
			httpresp.Fail(c, err)
		} else {
