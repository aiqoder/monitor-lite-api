...
func GetSelfoutHandler(svcCtx *svc.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req types.GetSelfoutByIdReq
		if err := bind.Parse(c, &req); err != nil {
			httpresp.Fail(c, err)
			return
		}

		l := selfout.NewGetSelfoutLogic(c.Request.Context(), svcCtx)
		resp, err := l.GetSelfout(&req)
		if err != nil {
			httpresp.Fail(c, err)
		} else {
