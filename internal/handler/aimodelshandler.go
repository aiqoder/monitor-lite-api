...
func AiModelsHandler(svcCtx *svc.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req types.AiModelsReq
		if err := bind.Parse(c, &req); err != nil {
			httpresp.Error(c, err)
			return
		}

		l := logic.NewAiModelsLogic(c.Request.Context(), svcCtx)
		resp, err := l.AiModels(&req)
		if err != nil {
			httpresp.Error(c, err)
		} else {
